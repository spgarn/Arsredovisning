import balanceAssetsSections, { BalanceAssetsSectionsInterface } from '../info/balanceSectionsAssetsData';
import balanceEquitySections, { BalanceSectionsEquityInterface } from '../info/balanceSectionEquityData';
...
function sumAssetsSection(
  ...
  return Object.entries(accounts)
    .reduce((sum, [accountId, account]) => {
      ...
      const balanceRow = Object.entries(balanceAssetsSections[section].children)
      ...
      current: 0,
      previous: 0,
      children: Object.fromEntries(Object.keys(balanceAssetsSections[section].children)
...
function calculateBalanceAssets(accounts: Accounts): Result {
  ...
  const financialFixedAssets = sumAssetsSection(accounts, 'financialFixedAssets');
  ...
  const prepaidCostsAndDelayedIncome = sumAssetsSection(accounts, 'prepaidCostsAndDelayedIncome');
...
function sumEquitySection(
  ...
  return Object.entries(accounts)
    .reduce((sum, [accountId, account]) => {
      const accountNumber = Number(accountId);
      const balanceRow = Object.entries(balanceEquitySections[section].children)
        .find(([, { accountRange: [min, max] }]) => {
          if (accountNumber >= min && accountNumber <= max) return true;
          return false;
        });
      ...
      current: 0,
      previous: 0,
      children: Object.fromEntries(Object.keys(balanceEquitySections[section].children)
        .map((title) => [title, { current: 0, previous: 0 }])),
    });
}
