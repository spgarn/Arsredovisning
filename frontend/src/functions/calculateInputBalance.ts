...
function calculateInputBalance(company: Company): void {
  return Object.entries(company.balance)
    .forEach(([section, data]) => {
      ...
      company.balance[section].current = Object.values(data.children)
        .reduce((total, { current }) => total + current, 0);
      // eslint-disable-next-line no-param-reassign
      company.balance[section].previous = Object.values(data.children)
...
export { calculateInputBalance as default, sumInputResults };
